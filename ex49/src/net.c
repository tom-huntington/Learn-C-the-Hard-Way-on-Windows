...
int nonblock(int fd)
{
    ...
    check(flags >= 0, "Cant set nonblocking.");
...
int attempt_listen(struct addrinfo *info)
{
    ...
    debug("created socket: %d", sockfd);

    // set the SO_REUSEADDR option on the socket
    rc = setsockopt(sockfd, SOL_SOCKET, SO_REUSEADDR, (char *)&bOptVal, sizeof(int));
...
int server_listen(const char *host, const char *port)
{
    ...
    check(rc == 0, "Failed to get address info for connect. WSAError: %d", WSAGetLastError());
