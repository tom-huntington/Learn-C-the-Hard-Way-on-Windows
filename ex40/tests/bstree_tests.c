...
char *test_traverse()
{
    ...
    mu_assert(traverse_called == 3, "Wrong count for traverse.");

    traverse_called = 0;
    rc = BSTree_traverse(map, traverse_fail_cb);
    mu_assert(rc == 1, "Failed to traverse.");
    mu_assert(traverse_called == 2, "Wrong count for traverse failed.");
...
char *test_fuzzing()
{
    ...
    BSTreeNode *node = store->root;

    while(node->left) {
        debug("%s", bdata((bstring)node->left->key));
        node = node->left;
    }
