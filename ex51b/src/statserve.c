...
bstring sanitize_location(bstring base, bstring path)
{
    ...
    debug("post-format: %s", bdata(attempt));
    bdestroy(encpath);
    return attempt;
...
int handle_store(Command *cmd, RingBuffer *send_rb, bstring path)
{
    ...
    if(info == NULL) {
        ...
        //fd = _open(bdata(location), O_WRONLY | O_CREAT);
        debug("location: %s", bdata(location));

        HANDLE hFile = CreateFile(bdata(location),
                GENERIC_WRITE,
                0,
                NULL,
                OPEN_ALWAYS,
        ...
        check(hFile != INVALID_HANDLE_VALUE, "Cannot open file for writing: %s, error: %ld", bdata(location), GetLastError());
        debug("hFile: %p", hFile);
        ...
        bErrorFlag = WriteFile(
                ...
                &dwBytesWritten,
                NULL);
        debug("post-WriteFile");
        check(bErrorFlag == TRUE, "Failed to write to %s, error: %ld", bdata(location), GetLastError());
        check((int)dwBytesWritten == sizeof(Stats), "didn't write all bytes, bytes written: %ld, length: %d", dwBytesWritten, blength(location));
...
int handle_load(Command *cmd, RingBuffer *send_rb, bstring path)
{
    ...
    if(info != NULL) {
        ...
        debug("pre-open read only");
        // open the file to read from readonly and locked
        fd = open(bdata(location), O_RDONLY);
        //HANDLE hFile = CreateFile(bdata(location),
        //        GENERIC_READ,
        //        FILE_SHARE_READ,
        //        NULL,
        ...
        //        FILE_ATTRIBUTE_NORMAL,
        //        NULL);
        //check(hFile != INVALID_HANDLE_VALUE, "unable to open file: %s, error: %ld", bdata(location), GetLastError());
        //check(fd >= 0, "Error opening file: %s", bdata(location));

        //DWORD nBytesRead;        
        //check(ReadFile(hFile, info->stat, sizeof(Stats), &nBytesRead, NULL), "didn't read file");
        //check(sizeof(Stats) == nBytesRead, "Didn't read sizeof(stats) bytes");

        // read into the stats record 
        rc = read(fd, info->stat, sizeof(Stats));
        check(rc == sizeof(Stats), "Failed to read record at %s rc: %d sizeof(stats): %d", bdata(location), rc, sizeof(Stats));
...
{
    TCHAR buffer[BUFF_SIZE] = TEXT("");
    // a more advanced design simply wouldn't use this
    DATA = Hashmap_create(NULL, NULL);
    check_mem(DATA);

    DWORD retval = GetFullPathName(store_path, BUFF_SIZE, buffer, NULL);

    //char *path = realpath(store_path, NULL);
    check(retval > 0, "Failed to get the real path for storage: %s", store_path);
    debug("full path: %s", buffer);
    
    STORE_PATH = bfromcstr(buffer);
...
int run_server(const char *host, const char *port, const char *store_path)
{
    ...
    int rc = setup_data_store(store_path);
