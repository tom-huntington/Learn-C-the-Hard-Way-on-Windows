...
struct tagbstring IN_STR = bsStatic(
        "I have ALPHA beta ALPHA oranges ALPHA");
...
char *test_find_and_scan()
{
    ...
    mu_assert(find_i > 0, "Failed to find ALPHA");
    ...
    mu_assert(find_i < scan_i, "should find another alpha after the first.");
...
char *test_find_performance()
{
    ...
    do {
        for (i = 0; i < 1000; i++) {
                ...
                check(found_at != -1, "string_find returned -1.");
                find_count++;
        }
